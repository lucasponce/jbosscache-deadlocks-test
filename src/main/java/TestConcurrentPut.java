...
public class TestConcurrentPut
{
    ...
    public static void main(final String... args) throws Exception
    {
        ...
        final Cache<String, String> cache = cacheFactory.createCache("test-jbosscache-config.xml", true);

        System.out.println("Wait until the cluster is ready");
        while (cache.getMembers() == null);
        while (cache.getMembers().size() < 2);
        ...
        Runnable task = new Runnable()
        {
            public void run()
            {
                ...
                while (true)
                {
                    try
                    {
                        String value = cache.get("/region", "a");
